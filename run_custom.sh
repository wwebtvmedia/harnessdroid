#!/bin/bash
cat << 'TEST' > app/src/androidTest/java/com/ai/harnessdroid/AgentLoopTest.kt
...
class AgentLoopTest {
    ...
    fun testLoopStopsWhenMaxTurnsReached() = runBlocking {
        ...
        if (!result.contains("Maximum turns reached") || mockToolRegistry.executionCount != 3) {
            throw java.lang.RuntimeException("RESULT WAS: " + result + " COUNT: " + mockToolRegistry.executionCount)
        }
    }
    ...
    fun testLoopSuccessfullyExecutesToolAndReturnsFinalAnswer() = runBlocking {
        ...
        if (!result.contains("Final Answer") || mockToolRegistry.executionCount != 1) {
            throw java.lang.RuntimeException("RESULT WAS: " + result + " COUNT: " + mockToolRegistry.executionCount)
        }
    }
}
...
adb shell am instrument -w -e class com.ai.harnessdroid.core.AgentLoopTest com.ai.harnessdroid.test/androidx.test.runner.AndroidJUnitRunner > test_output.txt
cat test_output.txt
